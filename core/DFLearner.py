...
class DFLearner(BaseLearner):
    ...
    def single_tower_operation(self, optim, tgt_image_dp, src_image_stack_dp, tgt_image_flow, src_image_stack_flow, tgt_image, src_image_stack, intrinsics, model_idx=0):
        ...
        with tf.name_scope("depth_prediction"):
            ...
            src_pred_disps, _ = disp_net_res50(src_image_stack_dp, is_training=True, reuse=True)
            if opt.scale_normalize:
                src_pred_disps = [self.spatial_normalize(disp) for disp in src_pred_disps]
           ...
           # cross_consistency_loss = 0
            ...
            for s in range(opt.num_scales):
                ...
                if opt.smooth_weight > 0 and s < 4:
                    ...
                    #for i in range(opt.num_source):
                    dp_smooth_loss += opt.smooth_weight*self.compute_edge_aware_smooth_loss(src_pred_disps[s], curr_src_image_stack)
                ...
                if s < 4:
                    ...
                    curr_proj_image_src2tgt, coords_tgt2src = projective_inverse_warp(
                        ...
                        pred_poses[:,0,:6], 
                        intrinsics[:,s,:,:])
                    curr_proj_depth_src2tgt = bilinear_sampler(src_pred_depths[s], coords_tgt2src)

                        # Pose: src->tgt, warp tgt->src
                    curr_proj_image_tgt2src, coords_src2tgt = projective_inverse_warp(
                        curr_tgt_image,
                        tf.squeeze(src_pred_depths[s], axis=3),
                        pred_poses[:,0,6:],
                        intrinsics[:,s,:,:])
                    ...
                    curr_proj_error_src = tf.abs(curr_proj_image_tgt2src-curr_src_image_stack)
                    ...
                    curr_pixel_loss, dp_proj_error_src = self.ternary_loss(curr_src_image_stack, curr_proj_image_tgt2src, curr_valid_mask_src, max_dists[s+1])
                    dp_pixel_loss += curr_pixel_loss*layer_weights[s]
                    ...
                    depth_error_src = tf.abs(curr_proj_depth_tgt2src-src_pred_depths[s])
    ...
    def collect_summaries(self):
        ...
        for s in range(opt.num_scales):
            ...
            #for i in range(opt.num_source):
                # For [0, 1]
            tf.summary.image('scale%d_source_image' % s,\
                tf.image.convert_image_dtype(self.src_image_stack_all[s], dtype=tf.uint8))
            if s < 4:
                tf.summary.image('scale%d_projected_image_src2tgt' % s,
                    tf.image.convert_image_dtype(self.proj_image_src2tgt_stack_all[s], dtype=tf.uint8))
                tf.summary.image('scale%d_projected_image_tgt2src' % s,
                    tf.image.convert_image_dtype(self.proj_image_tgt2src_stack_all[s], dtype=tf.uint8))
                tf.summary.image('scale%d_proj_error_tgt' % s ,
                    self.deprocess_image(tf.clip_by_value(self.proj_error_tgt_stack_all[s] - 1, -1, 1)))
                tf.summary.image('scale%d_proj_error_src' % s ,
                    self.deprocess_image(tf.clip_by_value(self.proj_error_src_stack_all[s] - 1, -1, 1)))
                    # Depth
    ...
    def train(self, opt):
        ...
        with tf.device('/cpu:0'):
            ...
            #    saver_dp   = tf.train.Saver([var for var in tf.trainable_variables() if ('depth_net_res50/Conv/weights' not in var.name and 'depth_net' in var.name) or 'pose_net' in var.name])
           ...
           # var_name = []
           # checkpoint_path = os.path.join('/visinf/home/sgoyal/Desktop/DF-Net-master/DF-Net-master/pretrained','cs_5frame_pre')
           # reader = pywrap_tensorflow.NewCheckpointReader(checkpoint_path)
           # var_to_shape_map = reader.get_variable_to_shape_map()
           # for key in var_to_shape_map:
           #     if key =='depth_net_res50/Conv/weights':
           #         var_name = reader.get_tensor(key)
           # var_new = np.concatenate((var_name,var_name),axis=2)
            #print(np.shape(var_name))
            ...
            #with tf.Session() as sess:
                ...
                if opt.continue_train:
                    ...
                    print(sess.run(self.tgt_pred_depth))
                    print(sess.run(self.src_pred_depths))
                    ...
                    #saver_flow.restore(sess, ckpt_flow)
                       ...
                       # saver.restore(sess,opt.ckpt_dp)
                    for var in tf.trainable_variables():
                            #if  'depth_net_res50/Conv/weights' in var.name:
                            #print(var.name)
                            #print(sess.run(var))
                        sess.run(var.initializer)
                        #shape = var.shape
                        #var_new = np.random.random(shape)
                        #var.load(var_new,sess)
                        #w = tf.get_variable("depth_net_res50/Conv/weights",[7,7,3,64])
                        #print(w.name)
                        #saver_dp.restore(sess, ckpt_dp)
                        #with tf.Graph().as_default() as graph:
                            #w  = tf.get_variable("depth_net_res50/Conv/weights",[7,7,3,64])
                            #print(w.name)
                            #sess.run(tf.assign(w,var_name))
                ...
                for step in range(1, opt.max_steps):
                    ...
                    if step % opt.summary_freq == 0:
                        ...
                        fetches["pxloss"] = self.dp_pixel_loss
                        fetches["smloss"] = self.dp_smooth_loss
                    ...
                    if step % opt.summary_freq == 0:
                        ...
                        print("Epoch: [%2d] [%5d/%5d] time: %4.4f/it loss: %.3f smoothloss: %.3f pixel loss: %.3f" \
                                % (train_epoch, train_step, self.steps_per_epoch, \
                                    (time.time() - start_time)/opt.summary_freq, 
                                    results['loss'], results['smloss'], results['pxloss']))
                        start_time = time.time()
